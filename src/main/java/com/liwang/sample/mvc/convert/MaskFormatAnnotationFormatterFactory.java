...
public class MaskFormatAnnotationFormatterFactory implements AnnotationFormatterFactory<MaskFormat> {
    ...
    public Printer<?> getPrinter(MaskFormat maskFormat, Class<?> aClass) {
        return new MaskFormatter(maskFormat.value());
    }

    public Parser<?> getParser(MaskFormat maskFormat, Class<?> aClass) {
        return new MaskFormatter(maskFormat.value());
    }
